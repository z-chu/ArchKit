...
fun KoinApplication.installAutoRegister() {
    ...
    installableServiceLoader.iterator().forEach {
        ...
        for (property in it.properties) {
            ...
            if (value != null) {
                if (koin.logger.isAt(Level.ERROR)) {
                    koin.logger.error("A property with the same key appears, please modify the key to prevent conflicts。key=${property.key},value1=${value}]，value2=${value}]。")
                }
            }
    ...
    if (installables != null) {
        koin.logger.debug("Koin auto install :${installables.map { it.javaClass.name }}")
    }
    modules(modules)
